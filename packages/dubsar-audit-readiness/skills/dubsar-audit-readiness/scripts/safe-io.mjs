...
export async function copyFileExclusive(sourceRoot, relativePath, outputRoot) {
  const source = safeChild(sourceRoot, relativePath);
  const target = safeChild(outputRoot, relativePath);
  const sourceInfo = await lstat(source);
  if (sourceInfo.isSymbolicLink() || !sourceInfo.isFile()) {
    throw new PublicPluginError("SOURCE_FILE_UNSAFE");
  }
  const realSource = await realpath(source);
  const sourcePrefix = `${comparablePath(sourceRoot)}${path.sep}`;
  if (!comparablePath(realSource).startsWith(sourcePrefix)) {
    throw new PublicPluginError("SOURCE_FILE_OUTSIDE_WORKSPACE");
  }
  ...
  try {
    ...
    throw new PublicPluginError("COPY_FAILED");
  }
}
...
export async function readRegularFile(root, relativePath, maxBytes) {
  const target = safeChild(root, relativePath);
  const info = await lstat(target);
  if (info.isSymbolicLink() || !info.isFile()) {
    throw new PublicPluginError("ARTIFACT_NOT_REGULAR_FILE");
  }
  if (Number.isFinite(maxBytes) && info.size > maxBytes) {
    throw new PublicPluginError("ARTIFACT_TOO_LARGE");
  }
  const resolved = await realpath(target);
  if (!isInsideOrEqual(root, resolved)) {
    throw new PublicPluginError("ARTIFACT_OUTSIDE_WORKSPACE");
  }
