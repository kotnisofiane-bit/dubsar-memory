...
import { REQUIRED_FILES, validateAuditWorkspace } from "./audit-model.mjs";
...
export async function currentAuditRootDigest(input) {
...
function validateReceiptShape(receipt, contextId, currentRoot) {
  ...
  if (receipt.format !== "dubsar.review-receipt/1" || receipt.context_kind !== "audit-case" || receipt.context_id !== contextId) fail("RECEIPT_CONTEXT_INVALID");
...
export async function recordAuditReviewReceipt(input, receipt) {
  const root = await openWorkspace(input);
  const validation = await validateAuditWorkspace(root);
  if (validation.status !== "valid") fail("WORKSPACE_INVALID");
  const currentRoot = await canonicalRoot(root);
  validateReceiptShape(receipt, validation.case_id, currentRoot);
  if (receipt.receipt_type === "reconciliation") await validateReviewedReceipts(root, receipt, validation.case_id);
