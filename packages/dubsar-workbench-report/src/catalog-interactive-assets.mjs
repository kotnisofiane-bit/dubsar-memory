...
(() => {
  ...
  function renderTickets() {
    ...
    ticketStates.forEach((state) => { const section = document.createElement("section"); const heading = document.createElement("h2"); heading.textContent = state; section.append(heading); const list = document.createElement("ul"); const matches = (data.tickets || []).filter((ticket) => ticket.state === state); matches.forEach((ticket) => { const item = document.createElement("li"); const launch = ticket.cursor_launch; item.textContent = ticket.id + " · " + ticket.title + " — " + ticket.project + (launch ? " · Cursor " + launch.agent_id + " / " + launch.run_id + " · " + launch.target_repository + " · " + launch.source_url : ""); list.append(item); }); if (matches.length === 0) { const empty = document.createElement("li"); empty.textContent = "Aucun ticket"; list.append(empty); } section.append(list); groups.append(section); });
