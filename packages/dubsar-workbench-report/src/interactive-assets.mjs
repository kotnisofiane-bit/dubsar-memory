...
(() => {
  ...
  function showTicket(ticket) {
    ...
    const launch = ticket.cursor_launch;
    const fields = [["Objectif", ticket.objective], ["Critères", (ticket.criteria || []).join(" • ")], ["Activité (ordre enregistré)", (ticket.activity || []).map((item) => "#" + item.index + " " + item.summary).join(" • ")], ["Agent / exécution", launch ? launch.agent_id + " / " + launch.run_id : (ticket.agent || "—")], ["Reçu Cursor", launch ? launch.receipt_version + " · " + launch.status + " · " + launch.target_repository_url + " · " + launch.source_url : "—"], ["Empreinte contrat", launch ? launch.contract_fingerprint : "—"], ["Branche", ticket.branch || "—"], ["PR", ticket.pr || "—"], ["Blocage", ticket.blocker || "—"], ["Références", (ticket.references || []).join(" • ") || "—"], ["Transitions disponibles", (transitionsByState[ticket.state] || []).join(" • ")]];
