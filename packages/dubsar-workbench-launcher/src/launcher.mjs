...
function parseTicketArguments(argv) {
  ...
  if (argv.at(0) !== "tickets" || !new Set(["list", "create", "transition", "activity", "attach-cursor-launch", "attach-cursor-run", "fail-cursor-launch", "fail-cursor-run", "attach-codex-launch", "attach-codex-run", "fail-codex-launch", "fail-codex-run", "record-codex-stop"]).has(action)) throw new TicketError("TICKET_CLI_ARGUMENT_INVALID");
