...
const receipt = { format: "dubsar.cursor-launch-receipt/1", ticket_id: "DUB-001", agent_id: "agent-1", run_id: "run-1", source_url: "https://cursor.example/runs/1", target_repository: "owner/repo", repository_refs: [{ ref: "main", sha: "a".repeat(40) }], contract_fingerprint: "b".repeat(64), bounds: { max_runs: 1, polling: false } };
...
test("skill impose allocation avant MCP, appel unique et arrêt sur contradiction", async () => {
  ...
  assert.match(skill, /Never call\n+   Cursor before this persisted allocation succeeds/); assert.match(skill, /exactly once/); assert.match(skill, /contract_fingerprint/); assert.match(skill, /Stop on a missing\/malformed receipt/); assert.match(skill, /do not poll/i);
});
